...
public class NXTRemoteControl extends Activity implements OnSharedPreferenceChangeListener, SensorEventListener {
    ...
    TextView heading;
    public float degree;
    public boolean temp_left=false,temp_right=false;
    public int leftval=110,rightval=160,rangebytwo=10;
    ...
    private void setupUI() {
        if (mControlsMode == MODE_BUTTONS) {
            ...
            heading = (TextView)findViewById(R.id.gyroData);
            ...
            btnTest.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					//Calibrate the left and right ranges according to current value of compass
					leftval = (int) degree-rangebytwo;
					rightval = (int) degree+rangebytwo;
					if(leftval<0) leftval = 360+leftval;
					if(rightval>360) rightval = rightval-360;
				}
			});
...
*/
	...
	public void onSensorChanged(SensorEvent event) {
        ...
        heading.setText("Heading: " + Float.toString(degree) + " degrees, dir="+(temp_left?"left":(temp_right?"right":"none")));
     
        float speed=0;
        float topspeedright=(float)0.3,topspeedleft=(float)0.5;
        
        if(angle_in_range(leftval,rightval,(int)degree))
        {
        	temp_left=false;
        	temp_right=false;
        	byte dir = (byte) ((byte)mPower*0);
//        	mNXTTalker.motors(dir_l, dir_r, mRegulateSpeed, mSynchronizeMotors);	
        	mNXTTalker.motor(-1,dir,mRegulateSpeed,mSynchronizeMotors);
        }
        //if the angle is outside the range and the angle is closer to the leftval than right
        else if(Math.abs(leftval-degree)<Math.abs(rightval-degree))        	
        {
        	temp_left=true;
        	temp_right=false;
        	float intensity = Math.max((Math.abs(leftval-degree)),40);
        	speed = (topspeedleft/40)*intensity;
        	byte dir = (byte) ((byte)mPower*(speed));
//        	mNXTTalker.motors(dir_l, dir_r, mRegulateSpeed, mSynchronizeMotors);	
        	mNXTTalker.motor(-1,dir,mRegulateSpeed,mSynchronizeMotors);
        }
        else if(Math.abs(leftval-degree)>Math.abs(rightval-degree))
        {
        	temp_right=true;
        	temp_left=false;
        	float intensity = Math.max((Math.abs(rightval-degree)),40);
        	speed = (topspeedright/40)*intensity;
        	byte dir = (byte) ((byte)mPower*-speed);
//        	mNXTTalker.motors(dir_l, dir_r, mRegulateSpeed, mSynchronizeMotors);	
        	mNXTTalker.motor(-1,dir,mRegulateSpeed,mSynchronizeMotors);
        }
        ...
        {
        	Log.i("NXTRemoteControl","If broken");
        }
	}
	
	boolean angle_in_range(int leftval, int rightval, int value)
	{
		if(leftval<=rightval) //normal
